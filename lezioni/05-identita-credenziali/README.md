...
personal access token: la policy enterprise fissa un massimo di **366 giorni**, e un PAT personale
nelle mani di un agente è l'anti-pattern per eccellenza — la lezione 2 ha già mostrato Anthropic che
scrive «Do not use a personal access token... could be partially or fully recovered over time via
prompt injection».
...
La stessa pagina *Security Best Practices* della spec MCP definisce un anti-pattern e lo vieta:
...
**OpenAI Codex cloud** — dalla documentazione degli environment: «Secrets are only available to
setup scripts. For security reasons, secrets are removed before the agent phase starts.» I segreti
servono a installare le dipendenze, poi spariscono. Attenzione alla distinzione che la pagina fa:
le **environment variables** normali restano per tutta la sessione; sono i *secrets* a essere
rimossi. E la rete, nella fase agente, è off di default.

**GitHub Copilot cloud agent** — dall'8 maggio 2026 esistono gli «Agents secrets and variables»,
a livello di repository o organizzazione, «exposed as environment variables in its development
...
articoli; è superato. Il token con cui l'agente parla a GitHub «is limited to the repository where
it's running»; i commit sono firmati, con Copilot come autore e l'umano come co-author.
...
workflow con un installation token dell'App — «short-lived token scoped specifically to the
repository». La API key di Anthropic, però, sta nell'ambiente del job, come secret di Actions:
esposta al processo. Esiste anche un'autenticazione OIDC **verso Anthropic** (workload identity
federation), che sostituisce la API key: è un secondo OIDC, con un altro scopo, da non confondere
con il primo. E un terzo modo, sconsigliato a livello di organizzazione: `CLAUDE_CODE_OAUTH_TOKEN`,
che è un token **personale** dell'utente.
...
Gennaio 2026, **Flatt Security** (RyotaK), su `claude-code-action`: il controllo sull'attore che
innesca l'action — solo chi ha write — poteva essere aggirato facendo passare l'attore per un
`[bot]`; ottenuto il trigger, l'attaccante rubava il token OIDC del workflow e lo scambiava con un
installation token con permessi di scrittura. Fix nella versione 1.0.94, **CVSS 7.8**. Il token
era short-lived e scoped, esattamente come da manuale; **il trigger non era controllato**. Chi può
innescare l'agente è la prima colonna della tabella dell'identità, non un dettaglio del workflow.
...
8. Anthropic, [`claude-code-action/docs/setup.md`](https://github.com/anthropics/claude-code-action/blob/main/docs/setup.md).
...
11. Flatt Security, [*Poisoning Claude Code: one GitHub issue to break the supply chain*](https://flatt.tech/research/posts/poisoning-claude-code-one-github-issue-to-break-the-supply-chain/), gennaio 2026.

Da riconoscere, non da leggere: la [policy GitHub sui personal access token](https://docs.github.com/en/enterprise-cloud@latest/admin/enforcing-policies/enforcing-policies-for-your-enterprise/enforcing-policies-for-personal-access-tokens-in-your-enterprise) — il massimo di 366 giorni.
...
| segreti nella fase agente | rimossi prima della fase agente | env var del processo agente | env var del job (API key), salvo OIDC verso Anthropic |
...
3. Mancava **il controllo su chi innesca**: facendosi passare per `[bot]` l'attaccante aggirava il
   check sull'attore, faceva partire il job e rubava il token OIDC per scambiarlo con un
   installation token in scrittura. Un token perfetto non protegge un trigger aperto: «chi può
